...
int xmin = 100, ymin = 200, xmax = 500, ymax = 500; //for clippingg window
bool clipLine = false;
...
// Line clipping for DDA
void drawLineDDAclipped() {
    int dx = x2_coord_DDA - x1_coord_DDA;
    int dy = y2_coord_DDA - y1_coord_DDA;

    float t1 = 0.0, t2 = 1.0;

    if (clipTest(-(x2_coord_DDA - x1_coord_DDA), x1_coord_DDA - xmin, t1, t2)) // Left
        if (clipTest((x2_coord_DDA - x1_coord_DDA), xmax - x1_coord_DDA, t1, t2)) // Right
            if (clipTest(-(y2_coord_DDA - y1_coord_DDA), y1_coord_DDA - ymin, t1, t2)) // Bottom
                if (clipTest((y2_coord_DDA - y1_coord_DDA), ymax - y1_coord_DDA, t1, t2)) { // Top
                    if (t2 < 1.0) {
                        x2_coord_DDA = x1_coord_DDA + t2 * dx;
                        y2_coord_DDA = y1_coord_DDA + t2 * dy;
                    }
                    if (t1 > 0.0) {
                        x1_coord_DDA = x1_coord_DDA + t1 * dx;
                        y1_coord_DDA = y1_coord_DDA + t1 * dy;
                    }

                    drawLineDDA(); //clipped line
                }
}
...
void drawLineBresenham() {
    ...
    //liang-barsky
    /*
    p1 = -(x1_coord_Bresenham - x2_coord_Bresenham);
    p2 = (x1_coord_Bresenham - x2_coord_Bresenham);
    p3 = -(y1_coord_Bresenham - y2_coord_Bresenham);
    p4 = (y1_coord_Bresenham - y2_coord_Bresenham);

    q1 = x1_coord_Bresenham - xmin;
    q2 = xmin - x1_coord_Bresenham;
    q3 = y1_coord_Bresenham - ymin;
    q4 = ymin - y1_coord_Bresenham;
    */
...
void displaySubWindow1() {
    ...
    if (isSecondClick_Bresenham) {
        if (clipLine) {
            drawLineDDAclipped(); // Draw clipped DDA line
        }
        else {
            drawLineDDA(); // Draw DDA line
        }
    }

    glColor3f(0.0, 0.0, 0.0);
    renderBitmapString(50, 30, GLUT_BITMAP_HELVETICA_18, "DDA / Cohen-Sutherland Line Algo");
...
void displaySubWindow2() {
    ...
    if (isSecondClick_Bresenham) {
        if (clipLine) {
            drawLineBresenhamClipped(); // Draw clipped Bresenham line
        }
        else {
            drawLineBresenham(); // Draw Bresenham line
        }
    }
